...
externes (IMDB, TMDB). ### Fonctionnalités disponibles :
...
### Bon à savoir
- Vous pouvez tester tous les endpoints directement via l'interface Swagger /*swager/
- Pour toute erreur (ex : ID inexistant), une réponse claire est retournée
avec le bon code HTTP."""

app = FastAPI(
    title= "MovieLens API by DIABONE",
...
@app.get (
    ...
    operation_id="health_chech_movies_api",
    tags=["monitoring"]
)

async def root():
    return {"message": "API MovieLens fonctionne correctement"}
...
#Endpoints pour obtenir une liste des films (avec des paginations et filtres facultatifs, title, genre, skip, limit)
...
#Endpoints pour obtenir une évaluation par utilisateur et film
